...
export function ModelSelector({ models }: ModelSelectorProps) {
  ...
  const handleModelSelect = (id: string) => {
    ...
    if (selectedModel) {
      // if selectedModel.provider == 'openrouter' -> set cookie as openrouter + other selected models
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          role="combobox"
          aria-expanded={open}
          className="text-sm rounded-full  h-10 shadow-none focus:ring-0 min-w-[220px] md:min-w-64"
        >
          {selectedModel ? (
            <div className="flex items-center gap-1.5 space-x-1">
              ...
              {isReasoningModel(selectedModel.id) && (
                <Lightbulb size={15} className="text-accent-blue-foreground" />
              )}
            </div>
          ) : (
            'Select model'
      ...
      <PopoverContent className="md:w-80 w-72 p-0" align="start">
        <Command>
